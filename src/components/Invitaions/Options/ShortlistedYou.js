...
const ShortlistedYou = ({ user }) => {
  ...
  useEffect(() => {
    ...
    fetch(process.env.REACT_APP_API_URL+`user/get_single_user_id/${userId}`)
      ...
      .then((res) => {
        var datatostore = [];
        let maxLength = res?.shortList.length;
        res.shortList.forEach((user) => {
          axios
            .get(
  ...
  return (
    ...
    >
      ...
      <div><h4>
        You were shortlisted by</h4> <hr /> </div>
      <div className="row">
        {data && data.length ? (
